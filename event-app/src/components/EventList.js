 ...
 // eslint-disable-next-line
  ...
  useEffect(() => {
    ...
    const fetchUserId = async () => {
        try {
            const response = await fetch('http://9.223.200.206:80/api/session', {
                credentials: 'include'
            });
  ...
  useEffect(() => {
    const fetchPublicEvents = async () => {
      try {
        ...
        const publicEventsResponse = await fetch('http://9.223.200.206:80/api/events/public', {
          credentials: 'include',
        });
    ...
    const fetchFriendsAndEvents = async () => {
      ...
      try {
          ...
          fetchedFriends.map(async (request) => {
            try {
              const userResponse = await fetch(
                `http://9.223.200.206:80/api/user/${request.id}/private-events`,
                { credentials: 'include' }
              );
              if (!userResponse.ok) {
        ...
        const userEventsResponse = await fetch(
          `http://9.223.200.206:80/api/user/${userId}/private-events`,
          { credentials: 'include' }
        );
