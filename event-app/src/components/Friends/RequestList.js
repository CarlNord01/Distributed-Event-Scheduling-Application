...
function RequestList() {
    ...
    useEffect(() => {
        ...
        const fetchFriendRequests = async () => {
            try {
                const response = await fetch(`http://9.223.200.206:5001/api/friend-requests/${userId}`, {
                    credentials: 'include'
                });
                ...
                const requestsWithUsernames = await Promise.all(requests.map(async (request) => {
                  try {
                      const userResponse = await fetch(`http://9.223.200.206:5001/api/user-summary/${request.sender}`, {
                          credentials: 'include'
                      });
