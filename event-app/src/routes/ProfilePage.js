...
function ProfilePage() {
  ...
  useEffect(() => {
    ...
    // Fetch user data
    axios.get(`http://backend:80/api/user/${userId}`)
      .then(response => {
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        if (isOwner) {
          ...
          const eventsResponse = await axios.get(`http://backend:80/api/user/${userId}/events`);
          ...
          if (friendStatus) {
            ...
            const eventsResponse = await axios.get(`http://backend:80/api/user/${userId}/public-events`);
            setEvents(eventsResponse.data);
          }
        }
      } catch (error) {
