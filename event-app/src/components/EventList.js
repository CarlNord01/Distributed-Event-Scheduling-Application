  ...
  useEffect(() => {
    const fetchPublicEvents = async () => {
      try {
        ...
        const publicEventsResponse = await fetch('http://9.223.200.206:5001/api/events/public', {
          credentials: 'include',
        });
    ...
    const fetchFriendsAndEvents = async () => {
      ...
      try {
          ...
          fetchedFriends.map(async (request) => {
            try {
              const userResponse = await fetch(
                `http://9.223.200.206:5001/api/user/${request.id}/private-events`,
                { credentials: 'include' }
              );
              if (!userResponse.ok) {
        ...
        const userEventsResponse = await fetch(
          `http://9.223.200.206:5001/api/user/${userId}/private-events`,
          { credentials: 'include' }
        );
