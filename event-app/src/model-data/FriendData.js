...
export const sendFriendRequest = async (targetUserId) => {
    try {
      ...
      axios.post(`http://backend:80/api/friend-request/${targetUserId}`, { withCredentials: true })
        .then((response) => {
  ...
  export const killFriend = async (victimId) => {
    try {
        const response = await fetch(`http://backend:80/api/friend/remove/${victimId}`, {
            method: 'POST',
            credentials: 'include',
        });
  ...
  export const getUserFriends = async () => {
    try {
      const response = await fetch(`http://backend:80/api/friends`, {
        credentials: 'include'
      });
  ...
  export const checkFriend = async (userId) => {
    try {
      const response = await fetch(`http://backend:80/api/friends/checkfriend/${userId}`, {
        credentials: 'include'
      });
  ...
  export const checkPendingRequest = async (userId) => {
    try {
      const response = await fetch(`http://backend:80/api/friends/checkfriend/request/${userId}`, {
        credentials: 'include'
      });
