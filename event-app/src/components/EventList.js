  ...
  useEffect(() => {
    const fetchPublicEvents = async () => {
      try {
        ...
        const publicEventsResponse = await fetch('http://backend:80/api/events/public', {
          credentials: 'include',
        });
    ...
    const fetchFriendsAndEvents = async () => {
      ...
      try {
          ...
          fetchedFriends.map(async (request) => {
            try {
              const userResponse = await fetch(
                `http://backend:80/api/user/${request.id}/private-events`,
                { credentials: 'include' }
              );
              if (!userResponse.ok) {
        ...
        const userEventsResponse = await fetch(
          `http://backend:80/api/user/${userId}/private-events`,
          { credentials: 'include' }
        );
